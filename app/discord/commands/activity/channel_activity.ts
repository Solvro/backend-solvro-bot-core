...
const command = new StaticCommand(
  new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt
        .setName("interval")
        .setDescription(
          "Time interval: week / month / 6 months (nothing = whole time)",
        )
        .addChoices(
  ...
  async (interaction: ChatInputCommandInteraction) => {
    ...
    const range = interaction.options.getString("interval") as
      | "week"
      | "month"
      | "6months"
      | null;
    ...
    const channels = await query.orderBy("message_count", "desc").limit(10);

    if (channels.length === 0) {
      await interaction.editReply({
        content: "Brak danych o aktywności kanałów dla wybranego przedziału.",
      });
      return;
    }
