...
export default async function handleMeetingTranscription(
  ...
  if (meeting?.recordingStatus !== RecordingStatus.COMPLETED) {
  ...
  const guild =
    interaction.guild ??
    (await client.guilds.fetch(env.get("DISCORD_GUILD_ID")));
  // Cache all users
