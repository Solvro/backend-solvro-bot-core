...
export default class CheckCamera extends BaseCommand {
  ...
  async run() {
    ...
    const thresholdMinutes = env.get(
      "OFFICE_CAMERA_DOWN_TIME_THRESHOLD_MINUTES",
      10,
    );
    ...
    this.logger.warning(
      `⚠️ Camera might be down! Status messages were not updated for more than ${thresholdMinutes} minutes.`,
    );
    ...
    for (const dmMessage of dmMessages) {
      try {
        ...
        await user.send(
          `⚠️ Warning: Office camera might be down! Status messages were not updated for more than ${thresholdMinutes} minutes.`,
        );
        this.logger.info(
          `Sent camera down notification to user ID: ${dmMessage.discordUserId} (username: ${user.username})`,
        );
