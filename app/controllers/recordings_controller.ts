    ...
    try {
      ...
      if (await googleDriveService.isConfigured()) {
        try {
          ...
          logger.info(
            `Successfully uploaded files to Google Drive for meeting ${id}`,
          );
        } catch (error) {
          logger.error(
            { err: error },
        ...
        logger.warn(
          `Google Drive not configured, skipping upload for meeting ${id}`,
        );
      }
      ...
      logger.error({ err: error }, "Error processing summary webhook");
