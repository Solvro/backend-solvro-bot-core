  ...
  async (interaction: ChatInputCommandInteraction) => {
    ...
    try {
      ...
      const attendeesInput =
        (interaction.options.get("attendees")?.value as string) || "";
      ...
      const attendees: { email: string }[] = attendeesInput
        ? attendeesInput
            .split(",")
      ...
      if (location === "office") {
        const eventStartDateTime =
          googleCalendarService.formatToRFC3339WithWarsawTime(date, startTime);
        const eventEndDateTime =
          googleCalendarService.formatToRFC3339WithWarsawTime(endDate, endTime);
        ...
        const locationKeyword =
          location === "office" ? "biuro" : "konferencyjna";
      ...
      const event: CalendarEvent = {
        ...
        start: {
          dateTime: googleCalendarService.formatToRFC3339WithWarsawTime(
            date,
            startTime,
          ),
