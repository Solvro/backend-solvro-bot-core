...
const command: StaticCommand = new StaticCommand(
  new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("object")
        .setDescription("Object to print")
        .setRequired(false),
    ),
  async (interaction: ChatInputCommandInteraction) => {
    ...
    try {
      ...
      const object =
        (interaction.options.get("object")?.value as string) || "_";
      const startDateInput = interaction.options.get("start-date")
        ?.value as string;
      ...
      const eventStartDateTime =
        googleCalendarService.formatToRFC3339WithWarsawTime(
