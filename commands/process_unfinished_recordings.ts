...
export default class ProcessUnfinishedRecordings extends BaseCommand {
  ...
  async run() {
    ...
    this.logger.info(
      `Found ${unfinishedRecordings.length} unfinished recordings to process.`,
    );
    ...
    for (const recording of unfinishedRecordings) {
      ...
      try {
        const response = await fetch(
          `${env.get("TRANSCRIBER_URL")}/process-recordings`,
          {
            method: "POST",
        ...
        if (response.ok) {
          this.logger.info(
            `Successfully triggered processing for recording ID: ${recording.id}`,
          );
        } else {
          this.logger.error(
